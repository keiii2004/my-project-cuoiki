<template>
    <div class="menu-container">
      ...
      <div v-if="product" class="product-detail">
        ...
        <div class="product-info">
          ...
          <button class="add-to-cart" @click="addToCart(product)" :disabled="product.quality === 0">Thêm vào giỏ hàng</button>
        </div>
      </div>
      <div v-else>
  ...
  const infoproduct = this.cart.find(item => item.id === product.id && item.type === 'nuoc');
...
getProductQuantityInCart(productId) {
      const itemInCart = this.cart.find(item => item.id === productId &&  item.type === 'nuoc' );
